...
var directories = function (path) {
  //var a;
  var arr = fs.readdir(path, function (err, file) {
    for(var i in file){
      arrDir.push(path + '/' + file[i]);
    }
    generateCommandString(arrDir);
  });
};
...
client.on('data', function (data) {
  if(data.indexOf('ASK') == 0){
    ...
    directories('D:/University/NodeJS/Folders');
  }
  else if(data == 'DEC'){
    client.destroy();
  }
